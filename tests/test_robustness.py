...
class TestPredicateRobustness:
    ...
    def test_greater_than_robustness(self):
        ...
        np.testing.assert_array_equal(computed.flatten(), expected)
        ...
        np.testing.assert_array_equal(computed_random.flatten(), expected)
...
class TestTemporalOperator:
    ...
    def test_always_operator_fixed_interval(self):
        ...
        always_formula = wp.Always(self.base_predicate, [0, 2])
    ...
    def test_eventually_operator_fixed_interval(self):
        ...
        eventually_formula = wp.Eventually(self.base_predicate, [0, 2])
    ...
    def test_temporal_without_interval(self):
        ...
        always_formula = wp.Always(self.base_predicate)
        always_formula.set_weights(signals=self.signal)
        ...
        eventually_formula = wp.Eventually(self.base_predicate)
        eventually_formula.set_weights(signals=self.signal)
    ...
    def test_always_weight_setting(self):
        ...
        for key in always_formula.weights.keys():
            ...
            assert np.all(always_formula.weights[key] == 1), f"Weights for {key} should be 1."
        ...
        # Verify the range
        for key in always_formula.weights.keys():
            assert np.all(always_formula.weights[key] >= 0.5) and np.all(
                always_formula.weights[key] <= 2.0
    ...
    def test_eventually_weight_setting(self):
        ...
        for key in eventually_formula.weights.keys():
            ...
            assert np.all(eventually_formula.weights[key] == 1), f"Weights for {key} should be 1."
        ...
        # Verify the range
        for key in eventually_formula.weights.keys():
            assert np.all(eventually_formula.weights[key] >= 0.5) and np.all(
                eventually_formula.weights[key] <= 2.0
            ), f"Weights for {key} should be in the range [0.5, 2.0]."
    ...
    def test_nested_temporal_operators(self):
        ...
        inner_eventually = wp.Eventually(self.base_predicate, [0, 3])
        outer_always = wp.Always(inner_eventually, [0, 2])
        ...
        for key in outer_always.weights.keys():
            assert np.all(outer_always.weights[key] == 1), f"Weights for {key} should be 1."
    ...
    def test_always_robustness(
        ...
        # Compute robustness
        formula = wp.Always(self.base_predicate, [0, 2])
        ...
        for i in range(self.Nsamples):
            expected_random[:, i] = np.min(
                weights.T[i, :] * (self.signal_data[:, 0:3, 0, 0] - self.comparison),
                axis=1,
            )

        np.testing.assert_array_equal(computed_random, expected_random)

        # Compute robustness no time interval
        formula = wp.Always(self.base_predicate)
        ...
        for i in range(self.Nsamples):
            expected_random[:, i] = np.min(
                weights.T[i, :] * (self.signal_data[:, :, 0, 0] - self.comparison),
                axis=1,
            )
    ...
    def test_eventually_robustness(self):
        formula = wp.Eventually(self.base_predicate, [0, 2])
        # Test uniform weights
        formula.set_weights(signals=self.signal)
        ...
        formula = wp.Eventually(self.base_predicate)
        # Test uniform weights
        formula.set_weights(signals=self.signal)
    ...
    def test_nested_temporal_robustness(self):
        ...
        # Create nested formula: Always[0,1] Eventually[0,1] (x >= 1.0)
        inner_eventually = wp.Eventually(self.base_predicate, [0, 2])
        outer_always = wp.Always(inner_eventually, [0, 3])
...
class TestLogicalOperatorRobustness:
    ...
    def test_and_operator_basic(self):
        ...
        and_formula = wp.And(self.predicate1, self.predicate2)
        ...
        for key in and_formula.weights.keys():
            ...
            assert np.all(and_formula.weights[key] == 1), f"Weights for {key} should be 1."
    ...
    def test_or_operator_basic(self):
        ...
        or_formula = wp.Or(self.predicate1, self.predicate2)
        ...
        for key in or_formula.weights.keys():
            ...
            assert np.all(or_formula.weights[key] == 1), f"Weights for {key} should be 1."
    ...
    def test_and_robustness(self):
        ...
        np.testing.assert_array_equal(computed.flatten(), expected.flatten())
        ...
        for i in range(10):
            ...
            expected_random[:, i] = np.min(w * values, axis=1)

        np.testing.assert_array_equal(computed_random.flatten(), expected_random.flatten())
    ...
    def test_or_robustness(self):
        ...
        for i in range(10):
            ...
            expected_random[:, i] = np.max(w * values, axis=1)
...
class TestCombinedOperators:
    ...
    def test_combined_and_or(self):
        ...
        or_formula = wp.Or(and_formula, pred3)
        ...
        np.testing.assert_array_equal(computed.flatten(), expected_or.flatten())
        ...
        for i in range(10):
            ...
            inner_value = np.min(w_and * values, axis=1)
            outer_value = np.column_stack((inner_value, (0.5 - self.signal1.value[:, 0, 0, 0])))

            expected_random[:, i] = np.max(w_or * outer_value, axis=1)
    ...
    def test_nested_temporal_logic_robustness(self):
        ...
        inner_eventually = wp.Eventually(self.predicate2, [2, np.inf])
        inner_always = wp.Always(self.predicate1)

        formula = wp.And(inner_always, inner_eventually)
        ...
        for i in range(10):
            ...
            inner_value = np.min(
                w_always * (self.signal_data[:, 0:5, 0, 0] - self.comparison1), axis=1
            )
            outer_value = np.max(
                w_eventually * (2 * self.comparison2 - self.signal_data[:, 2:5, 0, 1]),
                axis=1,
            )

            and_value = np.column_stack((inner_value, outer_value))
            expected_random[:, i] = np.min(w_and * and_value, axis=1)

        np.testing.assert_array_equal(computed_random.flatten(), expected_random.flatten())

    def test_logic_temporal_robustness(self):
        inner_and = wp.And(self.predicate1, self.predicate2)
        inner_eventually = wp.Eventually(inner_and, [0, 2])

        formula = wp.Always(inner_eventually)
        ...
        for i in range(10):
            ...
            for s in range(self.signal_data.shape[0]):
                ...
                for t in range(5):
                    ...
                    eventually_robustness[t] = np.max(w_eventually[: mint - t] * inters)

                expected_random[s, i] = np.min(w_always * eventually_robustness)
...
class TestEdgeCases:
    ...
    def test_same_predicates(self):
        ...
        formula = wp.Or(predicate, predicate)
        ...
        np.testing.assert_array_equal(robustness, expected)

        formula = wp.And(predicate, predicate)
        signals = (self.signal, self.signal)
